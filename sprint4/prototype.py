...
class Parser :
    ...
    def recognize_adress(self,path) :
        ...
        with fitz.open(path) as res :
            ...
            matches3 = pattern3.findall(res.load_page(0).get_text())
            if matches1 :
                result[0] = matches1
            if matches2 :
                result[1] = matches2[0]
    ...
    def recognize_adress__(self,chaine) :
        ...
        if matches1 :
            print(matches1,"XXXXXXXXXXXXXXXXXX")
            matches1[0][1].replace('\n','')
            result[0] = matches1
        if matches2 :
            #print(matches2[1],"XXXXXXXXXXXXXXXXXX")
            result[1] = matches2[0]
        if matches3 :
            print(matches3)
            result[2] = matches3[0]
    ...
    """
    fonction qui retourne la discussion
    input : path
    output : discussion en string
    """
    def findDiscussion(self,path) :
        result = ""
        b = True
...
def summarize(parser,src,dst) :
    ...
    for fichier in os.listdir(src) :
        ...
        with open(dfile,'w') as d :
            ...
            d.write("Titres : ")
            d.write(parser.findTitle(sfile))
            d.write("\n_____________________________\n")
            d.write("Auteurs : ")
            d.write(str(parser.getAuthor(sfile)))
            d.write("\n_____________________________\n")
            d.write("Abstract : ")
            d.write(str(parser.findAbstract(sfile)[0].strip()))
...
if __name__ == "__main__" :
    parser = Parser()
    print(parser.nearestPart(sys.argv[1]))
    #print(parser.findRefs(sys.argv[1]))
    """
    txt = parser.getAuthorZone(sys.argv[1])
    print(txt)
    print(parser.recognize_adress__(txt))
    print('______________________________________')
    #takeAuthors(txt)
    print(parser.recognize_adress(sys.argv[1]))
    """
    """
